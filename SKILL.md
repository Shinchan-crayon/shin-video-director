...
6. 如果已有 `runtime/director-plan.json`：进入渲染前视觉风格硬性门禁，先调用 `style-consultant`，再调用 `animation-preset-builder`。
7. 如果已有 `runtime/style-profile.json`：调用 `remotion-renderer` 生成 `runtime/preview.mp4`。
8. 如果用户确认 `runtime/preview.mp4`：调用 `remotion-renderer` 导出 `成品/成品.mp4`。
...
必须先调用 `remotion-video-director` 生成导演方案，再暂停并询问用户视觉风格，默认推荐：浅色财经 MG。用户选择或接受默认后，必须先调用 `style-consultant`，再调用 `animation-preset-builder` 生成 `runtime/style-profile.json`。
...
如果模板或渲染命令没有读取 `runtime/director-plan.json` 和 `runtime/style-profile.json`，或没有使用 scene 里的 `sceneType` / `motionVariant` / `visualModules` 以及导演方案里的 `directorType` / `shotStyle` / `cameraMotion`，要先修模板/命令，不要继续交付低质量预览。
